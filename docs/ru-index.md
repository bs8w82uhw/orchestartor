---
title: Документация (RU)
---

## Назначение

Русскоязычный раздел документации для операционных процессов и контрактов совместимости API.

## Основные документы

- [Политики, контракты и исполнение](ru-policies-contracts-execution.md)
- [Шаблон метода](ru-method-template.md)
- [Шаблон билета совместимости API](ru-api-compatibility-contract-ticket-template.md)
- [Отладка API и контракт совместимости](ru-api-debug-compatibility.md)
- [Тестирование через Docker Compose и WSL](ru-testing-docker-compose.md)
- [Синхронизация документации с Gramax](ru-gramax-sync.md)

## Примечание

Англоязычные документы остаются каноничными для кода и API-терминов.
RU-раздел поддерживается синхронно для команд и операторов.
