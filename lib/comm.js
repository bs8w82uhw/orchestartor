...
class Communication {
	...
	handleSocketMessage(socket, json) {
		...
		switch(cmd) {
			...
			case 'authenticate':
				...
				this.api.invoke( '/api/user/resume_session', data, function(resp) {
					if (!resp.code) {
						...
						socket.admin = !!(resp.user && resp.user.privileges && resp.user.privileges.admin);
						
						self.logDebug(6, "User socket has authenticated successfully: " + socket.id, { username: resp.username });
						socket.send( 'login', {} );
					}
					else {
						// login error
			...
			case 'search_job_files':
				...
				else this.logDebug(9, "OOPS!  SOCKET IS NOT AUTHED! " + socket.id);
			break;
	...
	doAdminBroadcastAll(cmd, data) {
		...
		this.logDebug(10, "Sending broadcast to admins: " + cmd, data);
	...
	doUserBroadcast(username, cmd, data) {
		...
		this.logDebug(10, "Sending broadcast to user: " + username + ": " + cmd, data);
