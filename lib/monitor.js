		...
		async.series(
			[
				...
				function(callback) {
					...
					if (server.info.cpu.manufacturer == '-') server.info.cpu.manufacturer = server.info.cpu.vendor;
					server.info.cpu.combo = ((server.info.cpu.manufacturer || server.info.cpu.vendor || 'Unknown') + ' ' + (server.info.cpu.brand || '')).trim();
