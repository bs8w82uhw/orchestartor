...
class Satellite {
	...
	fetch_satellite_core(args, callback) {
		...
		// airgap = use local filesystem, so we can easily ship out prebuilt packages
		if (this.config.getPath('airgap.enabled')) {
			fs.stat( storage_key, function(err, stats) {
				if (err) return self.doError('airgap', "Failed to stat local satellite binary: " + storage_key + ": " + err, callback);
				callback( "200 OK", { 'Content-Type': "application/gzip" }, fs.createReadStream(storage_key) );
			} );
			return;
		} // airgap
