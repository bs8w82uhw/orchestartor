	...
	api_create_alert(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			try {
				...
				return self.doError('alert', "Failed to compile expression: " + params.id + ": " + err, callback);
			}
			...
			try {
				...
				return self.doError('alert', "Failed to compile macro in message: " + params.id + ": " + err, callback);
			}
