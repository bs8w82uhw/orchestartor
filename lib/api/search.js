		...
		this.loadSession(args, function (err, session, user) {
			...
			var finish = function(metadata) {
				if (params.id) {
					...
					if (params.readme) {
						...
						self.request.get( url, { retries: 5 }, function(err, resp, data, perf) {
							if (err) return self.doError( 'marketplace', "Failed to fetch README: " + err, callback );
							callback({ code: 0, item, version: ver, text: data.toString() });
						});
						return;
					}
					else if (params.data) {
						...
						self.request.json( url, false, { retries: 5 }, function(err, resp, data, perf) {
			...
			fs.stat( cache_file, function(err, stats) {
				if (err || (stats.mtimeMs / 1000 < Tools.timeNow() - marketplace.ttl)) {
					...
					self.request.json( marketplace.metadata_url, false, { retries: 5 }, function(err, resp, metadata, perf) {
