...
class Admin {
	...
	api_admin_import_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var temp_file = Path.join( os.tmpdir(), 'opsrocket-import-temp-' + Tools.generateShortID() + '.txt' );
			var num_lines = 0;
			var line_idx = 0;
	...
	api_admin_export_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var filename = 'opsrocket-data-export-' + Tools.formatDate(Tools.timeNow(), '[yyyy]-[mm]-[dd]') + '-' + Tools.generateShortID() + '.txt.gz';
