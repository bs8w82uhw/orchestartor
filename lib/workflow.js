...
class Workflows {
	...
	continueWFController(opts) {
		...
		// automation manager policy check for controller "continue" stage (optional)
		if (this.automationManager && this.automationManager.enabled) {
			var policy_decision = this.evaluateAutomationPolicy({
				id: node.id,
				title: `Workflow controller continue gate: ${ctrl_type} (#${node.id})`,
				risk_level: node.data.risk_level || node.data.risk || 'medium',
				human_approved: !!node.data.human_approved
			}, {
				username: job.username || ''
			});
			
			state.automation_policy = {
				allowed: policy_decision.allowed,
				mode: policy_decision.mode,
				risk_level: policy_decision.risk_level,
				reason: policy_decision.reason,
				requires_human_approval: policy_decision.requires_human_approval,
				human_approved: policy_decision.human_approved
			};
			
			if (!policy_decision.allowed) {
				this.logWorkflow(job, node.id, `Automation policy denied controller continue stage: ${policy_decision.reason}`);
				state.error = true;
				return;
			}
			else if (policy_decision.reason && policy_decision.reason.match(/^Advisory mode:/)) {
				this.logWorkflow(job, node.id, `Automation policy advisory for controller continue stage: ${policy_decision.reason}`);
			}
		}
