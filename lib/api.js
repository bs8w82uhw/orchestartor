...
class API {
	...
	apiSetup() {
		...
		// intercept all api responses so we can set the HTTP response code
		this.api.on('response', function(args, resArgs) {
			if ((resArgs.length == 1) && (typeof(resArgs[0]) == 'object') && resArgs[0].code && args.request.url.match(/^[^\?]+\/v\d+(\?|$)/)) {
				switch (resArgs[0].code) {
					case 'api': resArgs[1] = "400"; break;
					case 'access': resArgs[1] = "403"; break;
					default: resArgs[1] = "500"; break;
				}
			}
		});
	}
