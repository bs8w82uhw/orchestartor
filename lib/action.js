...
class Actions {
	...
	runJobAction(job, action, callback) {
		...
		// automation manager policy check for actions (optional)
		if (this.automationManager && this.automationManager.enabled) {
			var action_risk = action.risk_level || action.risk || 'medium';
			if (action.type.match(/^(run_event|plugin|disable)$/)) action_risk = 'high';
			
			var policy_decision = this.evaluateAutomationPolicy({
				id: action.id || Tools.generateUniqueID('act'),
				title: `Action ${action.type} for job #${job.id}`,
			...
			if (!policy_decision.allowed) {
				action.code = 'policy';
				action.description = "Automation policy denied action: " + policy_decision.reason;
				action.details = "**Policy Decision:** " + policy_decision.reason;
				this.appendMetaLog(job, "Action denied by automation policy: " + action.type + " (" + policy_decision.reason + ")");
				this.logAction(6, "Automation Manager denied action", {
					job_id: job.id,
