...
class Jobs {
	...
	launchJob(job, callback) {
		...
		// automation manager policy check (optional)
		if (!job.test && this.automationManager && this.automationManager.enabled) {
			var policy_decision = this.evaluateAutomationPolicy({
				id: job.id,
				title: event.title || job.event,
				risk_level: job.risk_level || event.risk_level || category.risk_level || 'medium',
				human_approved: !!job.human_approved
			}, {
				username: job.username || ''
			});
			
			job.automation_policy = {
			...
			this.logJob(6, "Automation Manager policy decision for job launch", {
				job_id: job.id,
				event: job.event,
				risk_level: policy_decision.risk_level,
				allowed: policy_decision.allowed,
			...
			if (!policy_decision.allowed) {
				return callback( new Error("Automation policy denied job launch: " + policy_decision.reason) );
			}
		}
